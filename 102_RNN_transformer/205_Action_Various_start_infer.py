...
SO101 Multi-Start Action Chunking Inference on Unseen Initial Poses with GLFW Viewer
"""
...
def main():
    ...
    window = glfw.create_window(1200, 900, "SO101 Action Chunking Inference (Multi-Start)", None, None)
    if not window:
        glfw.terminate()
    ...
    # 에피소드마다 새로운 랜덤 시작점(+/- 10도)을 생성하여 검증
    def reset_random_eval_pose():
        joint_noise = (torch.rand(5) * 2.0 - 1.0) * DEG10_RAD
        gripper_noise = (torch.rand(1) * 2.0 - 1.0) * 0.005
        unseen_pose = base_pos1 + torch.cat([joint_noise, gripper_noise])

        mujoco.mj_resetData(mj_model, mj_data)
        mj_data.qpos[:DOF] = unseen_pose.numpy()
        mujoco.mj_forward(mj_model, mj_data)
        
        print(f"[*] 새 에피소드 시작 자세: {unseen_pose.numpy().round(3).tolist()}")
        return [mj_data.qpos[:DOF].copy() for _ in range(HISTORY)]

    state_buffer = reset_random_eval_pose()
    action_plan_queue = []
    step_cnt = 0

    print("\n" + "=" * 60)
    print(f"[*] 목표 도착점: [0.70, 0.50, -0.40, 0.60, -0.50, 0.035]")
    print(f"[*] Action Chunking: {CHUNK_SIZE}스텝 묶음 예측 기반 추론")
    print("=" * 60 + "\n")

    while not glfw.window_should_close(window):
        step_start = time.time()

        # K스텝마다 1회 추론하여 큐(Queue)에 적재
