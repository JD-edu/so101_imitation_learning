...
SO101 6-DoF Action Chunking (K=30) Training with Multi-Start Pose Randomization (+-10 deg)
"""
...
EPOCHS = 45
LR = 1e-3
MODEL_PATH = "action_chunking_transformer_multistart.pt"
XML_PATH = "./scene.xml"
...
# --- 1. 다양한 시작점(+-10도) -> 동일 도착점 데모 수집 ---
def generate_multistart_episodes(model, data):
    print(f"[*] 다양한 시작점(+-10도)에서 동일 도착점으로 향하는 100세트 P2P 데모 수집 중...")
    
    # 기준 시작점 및 고정 도착점
    base_pos1 = torch.tensor([-0.6, -0.4, 0.5, -0.3, 0.2, 0.01], dtype=torch.float32)
    target_pos2 = torch.tensor([ 0.7,  0.5, -0.4,  0.6, -0.5, 0.035], dtype=torch.float32)

    DEG10_RAD = 10.0 * (math.pi / 180.0)  # 10도 = 약 0.1745 rad

    all_states, all_actions = [], []
    for ep in range(NUM_EPISODES):
        if ep == 0:
            start_pos = base_pos1.clone()
        else:
            # 1~5축 회전 관절: 기준값 +- 10도 범위 유니폼 랜덤 오프셋
            joint_noise = (torch.rand(5) * 2.0 - 1.0) * DEG10_RAD
            # 6축 그리퍼: +- 0.005m 슬라이드 노이즈
            gripper_noise = (torch.rand(1) * 2.0 - 1.0) * 0.005
            start_pos = base_pos1 + torch.cat([joint_noise, gripper_noise])
        ...
        for step in range(STEPS_PER_EP):
            ...
            # 부드러운 가감속을 위한 Minimum Jerk / Quintic S-Curve
            smooth_s = 3.0 * (tau ** 2) - 2.0 * (tau ** 3)
            step_noise = (torch.rand(DOF) - 0.5) * 0.003
            
            # 각기 다른 시작점에서 동일한 target_pos2로 수렴
            target_ctrl = (1.0 - smooth_s) * start_pos + smooth_s * target_pos2 + step_noise
    ...
    def __init__(self, states, actions, history=HISTORY, chunk_size=CHUNK_SIZE):
        ...
        for ep in range(num_episodes):
            ...
            for t in range(history - 1, last_t + 1):
                # Input: [t-history+1 : t+1] (과거 궤적)
                # Output Chunk: [t+1 : t+1+chunk_size] (미래 K개 액션 묶음)
...
class ActionChunkTransformer(nn.Module):
    def __init__(self, dof=DOF, history=HISTORY, chunk_size=CHUNK_SIZE, d_model=64, nhead=4, num_layers=2):
        ...
        # Context Vector -> 미래 K개 스텝 궤적(chunk_size * dof) 일괄 생성
...
def run_glfw_eval(mj_model, mj_data, model):
    ...
    window = glfw.create_window(1200, 900, "SO101 Multi-Start Action Chunking (GLFW)", None, None)
    if not window:
        glfw.terminate()
    ...
    state_buffer = reset_robot_random_start()
    action_plan_queue = []
    step_cnt = 0

    print("\n[*] Action Chunking(K=30) Closed-Loop 제어 실행 (ESC/창 닫기로 종료)...")

    while not glfw.window_should_close(window):
        step_start = time.time()

        # Action Queue가 소진되었을 때만 신경망 추론 1회 호출 (K 스텝 주기)
...
def main():
    ...
    states, actions = generate_multistart_episodes(mj_model, mj_data)
    ...
    print("\n[*] Multi-Start Action Chunking Transformer 학습 시작...")
