"""
1_transformer_encoder_eval_glfw.py
SO101 Transformer Encoder Closed-Loop Inference with GLFW (Unseen Start Pose)
"""
...
def main():
    ...
    # 1. 모델 가중치 로드
    model = TransformerEncoderBC()
    try:
        model.load_state_dict(torch.load(MODEL_PATH, map_location="cpu"))
        print(f"[*] 가중치 로드 성공: {MODEL_PATH}")
    except FileNotFoundError:
        print(f"[!] 에러: {MODEL_PATH} 가 없습니다. 학습 코드를 먼저 실행하세요.")
        return
    ...
    window = glfw.create_window(1200, 900, "SO101 Transformer Encoder BC Inference (GLFW)", None, None)
    if not window:
        glfw.terminate()
